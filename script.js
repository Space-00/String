var Input = {
  ...
  setupTestSquid(1, 8);document.addEventListener("contextmenu", function(event) {
...
function setupSpider(size, legs) {
  ...
  var legNum = legs;
  var jointNum = 3;
  var legSpread = Math.PI * 0.7;
  
  for (var i = 0; i < legNum; i++) {
    var node = critter;
    var ang = legSpread * (i / (legNum - 1) - 0.5);
    for (var ii = 0; ii < jointNum; ii++) {
      var len = size * (ii == 0 ? 40 : 80) / jointNum;
      var node = new Segment(
        node,
        len, 
        ang * (ii == 0),
    ...
    var leg = new LegSystem(node, jointNum, size * 24, critter);
  }

  var body = new Segment(critter, size * 20, Math.PI, 0.1, 1);
  var head = new Segment(critter, size * 20, 0, 0.1, 1);
...
function setupSnake(size, segments) {
  reset();
  critter = new Creature(
    window.innerWidth / 2,
    window.innerHeight / 2,
...
function setupOctopus(size, arms) {
  reset();
  critter = new Creature(
    window.innerWidth / 2, 
    window.innerHeight / 2,
  ...
  var armNum = arms;
  var jointNum = 24;
  var armSpread = Math.PI * 1.2;

  for (var i = 0; i < armNum; i++) {
    var node = critter;
    var ang = armSpread * (i / (armNum - 1) - 0.5);
    for (var ii = 0; ii < jointNum; ii++) {
      node = new Segment(
        node,
        size * 100 / jointNum,
    ...
    var arm = new LimbSystem(node, jointNum, size * 40, critter);
  }

  var body = new Segment(critter, size * 40, 0, 0.1, 1);
